...
describe("fetchAndMergeData", () => {
  ...
  it("should return empty if all supplier data is empty", async () => {
    // Stub fetchData to return empty data for all suppliers
    sinon.stub(Helper, "fetchData").resolves([null, null, null]);
  ...
  it("should return refined data when data is fetched from all suppliers", async () => {
    ...
    sinon
      .stub(Helper, "fetchData")
      .onFirstCall()
      .resolves(acmeData)
      .onSecondCall()
      .resolves(patagoniaData)
      .onThirdCall()
      .resolves(paperfliesData);
